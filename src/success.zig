...
pub fn gen(allocator: std.mem.Allocator) !lightmix.Wave(T) {
    const LENGTH: usize = SAMPLE_RATE;

    const pop_first = try gen_tone(allocator, Scale.gen(.{ .code = .e, .octave = 4 }));
    defer pop_first.deinit();
    const pop_second = try gen_tone(allocator, Scale.gen(.{ .code = .a, .octave = 3 }));
    ...
    try composer.append(.{ .wave = pop_first, .start_point = 0 });
    try composer.append(.{ .wave = pop_second, .start_point = LENGTH / 4 * 3 });
    return try composer.finalize(.{});
}

fn gen_tone(allocator: std.mem.Allocator, frequency: T) !lightmix.Wave(T) {
    const LENGTH: usize = SAMPLE_RATE;
